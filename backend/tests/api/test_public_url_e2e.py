...
def org_a(s):
    ...
    r = s.post(f"{BASE_URL}/api/v1/identity/orgs",
               json={"name": "AOS A", "slug": slug}, timeout=20)
...
def org_b(s):
    ...
    r = s.post(f"{BASE_URL}/api/v1/identity/orgs",
               json={"name": "AOS B", "slug": slug}, timeout=20)
    assert r.status_code == 201, r.text
    return r.json(), slug
...
def test_bootstrap_slug_idempotency_returns_409(s, org_a):
    body, slug = org_a
    r = s.post(f"{BASE_URL}/api/v1/identity/orgs",
               json={"name": "Duplicate", "slug": slug}, timeout=15)
...
def test_get_orgs_me_with_token(s, org_a):
    ...
    r = s.get(f"{BASE_URL}/api/v1/identity/orgs/me",
...
def test_members_list_shows_founder_as_owner(s, org_a):
    ...
    sub = body["founder"]["subject_id"]
    token = _mint_dev_token(org_id=org_id, subject_id=sub, role="OWNER")
    r = s.get(f"{BASE_URL}/api/v1/identity/orgs/me/members",
              headers={"Authorization": f"Bearer {token}"}, timeout=15)
    assert r.status_code == 200
    members = r.json()
...
def test_invite_viewer_gets_403(s, org_a):
    ...
    r = s.post(f"{BASE_URL}/api/v1/identity/orgs/me/invites",
...
def test_invite_manager_can_create_and_accept(s, org_a):
    ...
    r2 = s.post(f"{BASE_URL}/api/v1/identity/invites/accept",
...
def test_invite_accept_bad_token_returns_404(s):
    r = s.post(f"{BASE_URL}/api/v1/identity/invites/accept",
               json={"token": "totally-not-a-real-token",
...
def test_cross_tenant_property_isolation(s, org_a, org_b):
    ...
    a_prop = {"address_line1": "1 Alpha Way", "city": "AlphaCity",
              "state": "TX", "postal_code": "75001"}
    rA = s.post(f"{BASE_URL}/api/v1/ontology/properties",
                headers={"Authorization": f"Bearer {a_tok}"},
                json=a_prop, timeout=15)
    assert rA.status_code == 201, rA.text
    created_a = rA.json()
    assert created_a["org_id"] == a_org_id

    # Seed a property under org B
    b_prop = {"address_line1": "2 Bravo Blvd", "city": "BravoCity",
              "state": "TX", "postal_code": "75002"}
    rB = s.post(f"{BASE_URL}/api/v1/ontology/properties",
                headers={"Authorization": f"Bearer {b_tok}"},
                json=b_prop, timeout=15)
    assert rB.status_code == 201, rB.text
    created_b = rB.json()
    assert created_b["org_id"] == b_org_id

    # Org A list must NOT see org B's property.
    listA = s.get(f"{BASE_URL}/api/v1/ontology/properties",
                  headers={"Authorization": f"Bearer {a_tok}"}, timeout=15)
    ...
    listB = s.get(f"{BASE_URL}/api/v1/ontology/properties",
                  headers={"Authorization": f"Bearer {b_tok}"}, timeout=15)
...
def test_property_write_denied_for_viewer(s, org_a):
    ...
    sub = body["founder"]["subject_id"]
    token = _mint_dev_token(org_id=org_id, subject_id=sub, role="VIEWER")
    r = s.post(f"{BASE_URL}/api/v1/ontology/properties",
               headers={"Authorization": f"Bearer {token}"},
               json={"address_line1": "x", "city": "y", "state": "TX",
                     "postal_code": "75001"},
               timeout=15)
    assert r.status_code == 403, r.text
