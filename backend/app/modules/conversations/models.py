...
class Conversation(IdMixin, TenantMixin, TimestampMixin, Base):
    ...
    urgency: Mapped[str] = mapped_column(String(16), nullable=False, default="normal")  # low|normal|urgent
...
class Message(IdMixin, TenantMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("channel IN ('sms','email','voice','mail','note')", name="ck_message_channel"),
        CheckConstraint(
    ...
    conversation_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("core.conversation.id", ondelete="CASCADE"), nullable=False, index=True)
