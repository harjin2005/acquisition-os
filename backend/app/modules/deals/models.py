...
class Deal(IdMixin, TenantMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"status IN ({','.join(repr(s) for s in DEAL_STATES)})",
            name="ck_deal_status",
        ),
        CheckConstraint(
    ...
    close_target_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    )  # assignment | wholetail | retail | brrrr
...
class Offer(IdMixin, TenantMixin, TimestampMixin, Base):
    ...
    deal_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("core.deal.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    )  # cash | conventional | dscr | hard_money
