...
def upgrade() -> None:
    ...
    with op.batch_alter_table("property", schema="licensed") as b:
        ...
        b.add_column(sa.Column("tax_delinquent", sa.Boolean(), nullable=False, server_default=sa.false()))
        b.add_column(sa.Column("last_sale_price", sa.Float(), nullable=True))
        b.add_column(sa.Column("last_sale_date", sa.DateTime(timezone=True), nullable=True))
        b.add_column(sa.Column("source_ref", sa.String(128), nullable=True))
        b.add_column(sa.Column("attributes", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")))
    ...
    op.create_table("owner",
        ...
        *_tenant_cols(),
        ...
        sa.CheckConstraint("entity_type IN ('person','entity','trust','unknown')", name="ck_owner_entity_type"),
        schema="core",
    )

    op.create_table("ownership_link",
        _uuid_pk(),
        *_tenant_cols(),
        sa.Column("owner_id", UUID(as_uuid=True), sa.ForeignKey("core.owner.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.UniqueConstraint("org_id", "owner_id", "property_id", name="uq_ownership_link"),
        schema="core",
    )
    ...
    op.create_table("contact",
        ...
        sa.Column("display_name", sa.String(200), nullable=False),
        sa.Column("role", sa.String(32), nullable=False, server_default="owner_of_record"),
        sa.Column("owner_id", UUID(as_uuid=True), sa.ForeignKey("core.owner.id"), nullable=True),
        schema="core",
    )
    op.create_table("contact_channel",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("contact_id", UUID(as_uuid=True), sa.ForeignKey("core.contact.id", ondelete="CASCADE"), nullable=False),
        sa.Column("channel", sa.String(16), nullable=False),
        sa.Column("address", sa.String(400), nullable=False),
        sa.Column("confidence", sa.Float(), nullable=False, server_default="0.5"),
        sa.Column("provenance", sa.String(64), nullable=False, server_default="unknown"),
        sa.CheckConstraint("channel IN ('sms','voice','email','mail')", name="ck_channel_type"),
        sa.UniqueConstraint("org_id", "contact_id", "channel", "address", name="uq_channel_address"),
        schema="core",
    )
    op.create_table("consent_record",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("channel_id", UUID(as_uuid=True), sa.ForeignKey("core.contact_channel.id", ondelete="CASCADE"), nullable=False),
        sa.Column("state", sa.String(20), nullable=False, server_default="consent_unknown"),
        sa.Column("basis", sa.String(200), nullable=True),
        sa.Column("recorded_at", sa.DateTime(timezone=True), nullable=False),
        sa.UniqueConstraint("org_id", "channel_id", name="uq_consent_channel"),
        sa.CheckConstraint("state IN ('consent_unknown','prior_express','ebr','opted_out','dnc')", name="ck_consent_state"),
        schema="core",
    )

    # ---- core.buy_box ----------------------------------------------------
    op.create_table("buy_box",
        _uuid_pk(), *_tenant_cols(),
    ...
    op.create_table("motivation_signal",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("property_id", UUID(as_uuid=True), nullable=False),
        sa.Column("kind", sa.String(24), nullable=False),
        sa.Column("confidence", sa.Float(), nullable=False, server_default="1.0"),
        sa.Column("provenance", sa.String(128), nullable=False, server_default="ingested"),
    ...
    op.create_index("ix_motsig_property", "motivation_signal", ["org_id", "property_id"], schema="derived")

    op.create_table("metro_coverage",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("metro", sa.String(64), nullable=False),
        sa.Column("status", sa.String(16), nullable=False, server_default="waitlist"),
        sa.Column("disclosure_state", sa.Boolean(), nullable=False, server_default=sa.true()),
        ...
        sa.CheckConstraint("status IN ('live','beta','waitlist','none')", name="ck_metro_status"),
        schema="derived",
    )
    ...
    op.create_table("lead",
        ...
        sa.Column("reason_chips", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
    ...
    op.create_index("ix_lead_property_id", "lead", ["org_id", "property_id"], schema="core")

    op.create_table("lead_event",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("lead_id", UUID(as_uuid=True), sa.ForeignKey("core.lead.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_lead_event_lead", "lead_event", ["org_id", "lead_id"], schema="core")
    ...
    op.create_table("deal",
        ...
        sa.CheckConstraint("strategy IN ('flip','rental','brrrr')", name="ck_deal_strategy"),
        schema="core",
    )
    ...
    op.create_table("offer",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("deal_id", UUID(as_uuid=True), sa.ForeignKey("core.deal.id", ondelete="CASCADE"), nullable=False),
        sa.Column("version", sa.Integer(), nullable=False, server_default="1"),
    ...
    op.create_table("deal_checklist",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("deal_id", UUID(as_uuid=True), sa.ForeignKey("core.deal.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.CheckConstraint("status IN ('todo','in_progress','done','waived')", name="ck_checklist_status"),
        schema="core",
    )
    op.create_index("ix_checklist_deal", "deal_checklist", ["org_id", "deal_id"], schema="core")
    ...
    op.create_index("ix_conv_contact", "conversation", ["org_id", "contact_id"], schema="core")
    op.create_index("ix_conv_lead", "conversation", ["org_id", "lead_id"], schema="core")

    op.create_table("message",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("conversation_id", UUID(as_uuid=True), sa.ForeignKey("core.conversation.id", ondelete="CASCADE"), nullable=False),
        sa.Column("channel", sa.String(16), nullable=False),
        ...
        sa.Column("consent_snapshot", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        ...
        sa.CheckConstraint("channel IN ('sms','email','voice','mail','note')", name="ck_message_channel"),
        sa.CheckConstraint(
    ...
    op.create_index("ix_msg_conv", "message", ["org_id", "conversation_id"], schema="core")
    op.create_index("ix_msg_campaign", "message", ["org_id", "campaign_id"], schema="core")
    ...
    op.create_table("template",
        ...
        sa.CheckConstraint("channel IN ('sms','email','mail')", name="ck_template_channel"),
        schema="core",
    )
    op.create_table("campaign",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("name", sa.String(200), nullable=False),
        sa.Column("audience_query", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        ...
        sa.Column("template_ids", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("cadence", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_table("suppression",
        ...
        sa.CheckConstraint("channel IN ('sms','voice','email','mail','all')", name="ck_suppression_channel"),
        schema="core",
    )
    op.create_index("ix_suppression_addr", "suppression", ["org_id", "address"], schema="core")

    # ---- derived.underwriting_run + comp --------------------------------
    op.create_table("underwriting_run",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("property_id", UUID(as_uuid=True), nullable=False),
        sa.Column("lead_id", UUID(as_uuid=True), nullable=True),
        sa.Column("strategy", sa.String(16), nullable=False, server_default="flip"),
        ...
        sa.Column("model_version", sa.String(32), nullable=False, server_default="rulebase-v1"),
        sa.Column("agent_run_id", UUID(as_uuid=True), nullable=True),
        sa.CheckConstraint("strategy IN ('flip','rental','brrrr')", name="ck_uw_strategy"),
        sa.CheckConstraint("status IN ('draft','ready','insufficient_evidence','failed')", name="ck_uw_status"),
        schema="derived",
    )
    op.create_index("ix_uw_property", "underwriting_run", ["org_id", "property_id"], schema="derived")

    op.create_table("comp",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("run_id", UUID(as_uuid=True), sa.ForeignKey("derived.underwriting_run.id", ondelete="CASCADE"), nullable=False),
        sa.Column("property_id", UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("adjustment_details", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_table("agent_run",
        ...
        sa.CheckConstraint("agent_name IN ('prioritization','underwriting','followup')", name="ck_agent_name"),
        sa.CheckConstraint("status IN ('running','ok','failed','refused','human_review')", name="ck_agent_status"),
        schema="derived",
    )
    op.create_index("ix_agent_run_name", "agent_run", ["org_id", "agent_name", "started_at"], schema="derived")

    # ---- core.import_job + derived.import_quarantine --------------------
    op.create_table("import_job",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("dialect", sa.String(32), nullable=False, server_default="generic_csv"),
        ...
        sa.Column("consent_attestation", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("started_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("finished_at", sa.DateTime(timezone=True), nullable=True),
    ...
    op.create_index("ix_import_content_hash", "import_job", ["org_id", "content_hash"], schema="core")

    op.create_table("import_quarantine",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("job_id", UUID(as_uuid=True), nullable=False),
        sa.Column("row_num", sa.Integer(), nullable=False),
        sa.Column("row_data", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("reasons", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        schema="derived",
    )
    op.create_index("ix_quarantine_job", "import_quarantine", ["org_id", "job_id"], schema="derived")
    ...
    op.create_index("ix_audit_action", "log", ["org_id", "action", "created_at"], schema="audit")

    op.create_table("impersonation",
        _uuid_pk(), *_tenant_cols(),
        sa.Column("started_at", sa.DateTime(timezone=True), nullable=False),
    ...
    op.create_table("outbox",
        ...
        sa.CheckConstraint("status IN ('pending','published','failed','dead')", name="ck_outbox_status"),
        schema="events",
    )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("property", schema="licensed") as b:
        for col in ("attributes", "source_ref", "last_sale_date", "last_sale_price", "tax_delinquent",
                    "assessed_value", "year_built", "lot_sqft", "sqft", "baths", "beds", "apn", "county"):
            b.drop_column(col)
